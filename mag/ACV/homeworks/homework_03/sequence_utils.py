...
class VOTSequence:
    ...
    def load_sequence(self):
        ...
        self.gt = self.read_groundtruth(
            os.path.join(self.sequence_path, "groundtruth.txt")
        )
    ...
    def convert_region(self, region, type):
        if (len(region) == 4 and type == "rectangle") or (
            len(region) == 8 and type == "polygon"
        ):
            return region
    ...
    def draw_particles(self, img, particles):
        for x, y in particles:
            cv2.circle(
                img, (int(x), int(y)), radius=1, color=(0, 255, 0, 0.3), thickness=-1
            )
    ...
    def draw_region(self, img, region, color, line_width):
        if len(region) == 4:
            ...
            br = (
                int(round(region[0] + region[2] - 1)),
                int(round(region[1] + region[3])),
            )
    ...
    def draw_text(self, img, text, text_pos):
        ...
        cv2.putText(
            img, text, text_pos, font, 1, (255, 255, 255), 1, cv2.LINE_AA, False
        )
