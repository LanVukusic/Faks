...
def evaluate_tracker(dataset_path:str, network_path:str, results_dir:str, visualize:bool, trackerModel=TrackerSiamFC, sequences=None):
    ...
    tracker = trackerModel(net_path=network_path)
    ...
    for sequence_name in sequences:
        ...
        for i in tqdm(range(1, sequence.length()), desc=sequence_name):
            img = cv2.imread(sequence.frame(i))
            prediction, score, visible, tresh = tracker.update(img)
            
            results.append(prediction)
            scores.append([score])
            visible_ar.append(visible)
            tresh_ar.append(tresh)
    ...
    return results, scores, visible_ar, tresh_ar
