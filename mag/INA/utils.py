...
def read_edgelist(
    filename: str, data_folder=DEFAULT_DATA_FOLDER, progress_bar=False
) -> nx.Graph:
...
def read_pajek(
    ...
    label_parser: Callable[[str, str], Dict[str, Any]] = None,
) -> nx.Graph:
    ...
    if label_parser is None:
        ...
        def label_parser(lab, val):
            return {"label": lab} if val is None else {"label": lab, "value": val}
        ...
        for line in file:
            if line.startswith("*"):
                match line.split()[0][1:]:  # TODO extract m for optional progressbar
                    ...
                    case "arcs":
                        G = nx.MultiDiGraph(name=filename)
                    case link_type:
                        raise SyntaxError(f"invalid link type: {link_type}")
                break
            else:  # add node
...
def info(
    G: nx.Graph,
    distance_sample: int | None = None,
    clustering_sample: int | None = 10_000,
) -> None:
    ...
    print("{:>12s} | {:,d} (iso={:,d})".format("Nodes", n, nx.number_of_isolates(G)))
    print("{:>12s} | {:,d} (loop={:,d})".format("Edges", m, nx.number_of_selfloops(G)))

    if G.is_directed():
        print(
            "{:>12s} | {:.2f} max(in={:d}, out={:d})".format(
                "Degree",
                m / n,
                max(k for _, k in G.in_degree()),
    ...
    print(
        "{:>12s} | {:.1f}% (n={:,d})".format("LCC", 100 * len(largest_comp) / n, len(C))
    )
...
def top_nodes(G: nx.Graph, C: Dict[Any, float], centrality: str, n=15) -> List[Any]:
    ...
    nodes = []
    for i, c in sorted(
        C.items(), key=lambda item: (item[1], G.degree[item[0]]), reverse=True
    ):
...
def top_edges(
    G: nx.Graph, C: Dict[Tuple[Any, Any], float], centrality: str, n=15
) -> List[Any]:
    ...
    for (i, j), c in sorted(C.items(), key=lambda item: item[1], reverse=True)[:n]:
        line = "{:>12.6f} | '{:s}' - '{:s}'".format(
            c, G.nodes[i]["label"], G.nodes[j]["label"]
        )

        try:
            line += "({:,.0f})".format((G.nodes[i]["value"] + G.nodes[j]["value"]) / 2)
        except:
            pass  # no numeric values
        print(line)
    print()
