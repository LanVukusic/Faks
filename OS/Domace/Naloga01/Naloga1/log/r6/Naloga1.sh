...
case $1 in
    ...
    "stej")
        cat $( echo $@ | cut -d' ' -f2- ) | grep -v "#" $2 | cut -d : -f2 | sort | uniq -c | sort -nr | nl
        
    ;;

    "fib")
        for i in $(echo $@ | cut -d' ' -f2- | rev)
        do
            echo "$i: $(fib $i)"
        done
    ;;
    ...
    "upori")
        ...
        do  
            ...
            if [ ${#A} -ge 1 ]; then
                ...
                home=$(grep -E "^$i:" /etc/passwd | cut -d: -f4)

                enaka=$([[ $gid == $uid ]] && echo "enaka" || echo "razlicna")
                obstaja=$([[ $home == "" ]] && echo "ne-obstaja" || echo "obstaja")
